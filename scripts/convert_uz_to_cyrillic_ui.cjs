...
function processFile(filePath) {
  ...
  const source = ts.createSourceFile(filePath, original, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  const replacements = [];
...
if (!root) {
  console.error('Usage: node convert_uz_to_cyrillic_ui.cjs <dir>');
  process.exit(1);
}
walk(root);
